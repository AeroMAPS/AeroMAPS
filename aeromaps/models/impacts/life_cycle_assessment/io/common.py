...
class Lambda:
    ...
    def __json__(self):
        ...
        return dict(params=self.params, expr=expr)
...
def unexpand_param_names(all_params, expanded_param_names):
    ...
    expanded_params_to_params = {
        name: param.name for param in all_params.values() for name in param.expand_names()
    }
...
class Model:
    ...
    def evaluate(
        self, impact, functional_unit=FUNCTIONAL_UNIT_KEY, axis=TOTAL_AXIS_KEY, **param_values
    ):
        """
        ...
        if axis not in self.expressions:
            raise Exception(
                "Wrong axis '%s'. Expected one of %s" % (axis, list(self.expressions.keys()))
            )

        expressions_by_impact = self.expressions[axis]

        if impact not in expressions_by_impact:
            raise Exception(
                "Wrong impact '%s'. Expected one of %s"
                % (impact, list(expressions_by_impact.keys()))
            )
        ...
        if isinstance(impacts, dict):
            # Filter out "null"=zero axis
            impacts = {
                key: val for key, val in impacts.items() if not (key == "null" and val == 0.0)
            }
    ...
    def from_json(cls, js, axis=None, methods=None):
        ...
        if axis is not None:  # Get only one axis if specified
            if axis not in js["expressions"]:
                warnings.warn(
                    f"Axis '{axis}' not found in model expressions. Using first available axis instead."
                )
        ...
        expressions = {
            axis: {method: Lambda.from_json(lambd, all_params) for method, lambd in impacts.items()}
            for axis, impacts in expr_items
        }

        functional_units = {
            key: FunctionalUnit(
                quantity=Lambda.from_json(fu["quantity"], all_params), unit=fu["unit"]
            )
        ...
        if methods is not None:
            filtered_impacts = {
                key: impact_metadata
                for key, impact_metadata in js["impacts"].items()
                if key in methods
            }
        impacts = {}
    ...
    def from_json_with_progress_bar(cls, js, axis=None, methods: list = None):
            ...
            for ax, impacts_dict in expr_items:
                for method in methods:
                    if method not in impacts_dict:
                        warnings.warn(
                            f"Method '{method}' not found in axis '{ax}' expressions. Skipping."
                        )
        ...
        for ax, impacts in expr_items:
            expressions[ax] = {}
            for method, lambd in tqdm(
                impacts.items(), desc=f"Import LCIA functions (axis '{ax}')", leave=True
            ):
