        ...
        for i in range(year, year + int(aircraft_life)):
            if i < (self.end_year + 1):
                discounted_cumul_cost += (
                                                 extra_cost_non_fuel
                                                 + extra_cost_fuel[year] * kerosene_market_price[i] /
...
class CargoEfficiencyCarbonAbatementCosts(AeromapsModel):
    def __init__(self, name="cargo_efficiency_carbon_abatement_cost",fleet_model=None, *args, **kwargs):
    ...
    def compute(
        ...
        freight_reference_energy = energy_per_rtk_without_operations_freight_dropin_fuel.loc[self.prospection_start_year - 1]
        ...
        extra_emissions_dropin = (-aircraft_energy_delta_dropin * kerosene_emission_factor) / 1000000
        extra_emissions_hydrogen = (-aircraft_energy_delta_hydrogen * kerosene_emission_factor) / 1000000
        extra_emissions_electric = (-aircraft_energy_delta_electric * kerosene_emission_factor) / 1000000
        ...
        self.df.loc[:, "aircraft_carbon_abatement_cost_freight_dropin"] = aircraft_carbon_abatement_cost_freight_dropin
        self.df.loc[:,
        "aircraft_carbon_abatement_cost_freight_hydrogen"] = aircraft_carbon_abatement_cost_freight_hydrogen
        self.df.loc[:,
        "aircraft_carbon_abatement_cost_freight_electric"] = aircraft_carbon_abatement_cost_freight_electric
        ...
        self.df = pd.concat([self.df, aircraft_specific_carbon_abatement_cost_freight_dropin], axis=1)
        self.df = pd.concat([self.df, aircraft_generic_specific_carbon_abatement_cost_freight_dropin], axis=1)
        ...
        self.df = pd.concat([self.df, aircraft_specific_carbon_abatement_cost_freight_hydrogen], axis=1)
        self.df = pd.concat([self.df, aircraft_generic_specific_carbon_abatement_cost_freight_hydrogen], axis=1)
        ...
        self.df = pd.concat([self.df, aircraft_specific_carbon_abatement_cost_freight_electric], axis=1)
        self.df = pd.concat([self.df, aircraft_generic_specific_carbon_abatement_cost_freight_electric], axis=1)
        ...
        self.df.loc[:,
        "aircraft_carbon_abatement_volume_freight_hydrogen"] = aircraft_carbon_abatement_volume_freight_hydrogen
        self.df.loc[:,
        "aircraft_carbon_abatement_volume_freight_electric"] = aircraft_carbon_abatement_volume_freight_electric
