...
class ElectrofuelCost(AeroMAPSModel):
    ...
    def compute(
        ...
        print("Total EF TIME:", timeit.default_timer() - t0)
    ...
    def _electrofuel_computation(
        ...
        electrofuel_production  = np.zeros(len(indexes))
        ...
        electrofuel_total_cost = np.zeros(len(indexes))
        electrofuel_capex_cost = np.zeros(len(indexes))
        electrofuel_opex_cost = np.zeros(len(indexes))
        electrofuel_elec_cost = np.zeros(len(indexes))
        electrofuel_co2_cost = np.zeros(len(indexes))
        ...
        kerosene_market_price_array = kerosene_market_price.values
        ...
        for year in list(demand_scenario.index):
            ...
            if (year + 1) <= self.end_year and electrofuel_production[
                ...
                electrofuel_mfsp = self._compute_electrofuel_year_lcop(
                ...
                missing_production = demand_scenario[year + 1] - electrofuel_production[year - self.historic_start_year + 1]
                ...
                for i in range(year, year + int(plant_lifespan)):
                    ...
                    if i < (self.end_year + 1):
                        ...
                        discounted_cumul_cost += (
                                                         electrofuel_mfsp[self.end_year - year, 0] -
                ...
                generic_specific_carbon_abatement_cost[year] = (
                    ...
                    electrofuel_production[year - self.historic_start_year + 1] >= demand_scenario[year + 1] > 0
            ):
        ...
        # Abatement cost in €/tCO2e (= overcost for a ton of electrofuel/avoided emissions)
        carbon_abatement_cost_electrofuel = (
                (electrofuel_mean_mfsp_litre - kerosene_market_price)
    ...
    def _compute_electrofuel_year_lcop(
        ...
        load_fact = np.minimum(plant_load_fact, electricity_load_factor[technology_year])
        ...
        electrofuel_efficiency = (
                electrolysis_efficiency[technology_year]
                * electrofuel_hydrogen_efficiency[technology_year]
                * (0.81 + 0.03 + 0.06)
                / (0.81 + 0.03)
        )
        ...
        # Calculate NPV for fixed operating costs, variable operating costs, and hydrogen production.
        years_operation = np.arange(construction_time, plant_lifespan + construction_time)
        fix_op_cost_npv = np.sum(
            (electrofuel_fixed_opex[technology_year] * density_electrofuel)
        ...
        electrofuel_prices_array = np.zeros((num_years, 6))

        # Electricity cost for each year
        elec_prices = electricity_market_price.loc[
                      base_year + construction_time: end_bound
                      ].values * electrofuel_specific_electricity

        # CO2 cost for each year
        co2_costs = electrofuel_specific_co2[technology_year] * co2_market_price.loc[
                                                                base_year + construction_time: end_bound
                                                                ].values

        # Total electrofuel prices for each year
        electrofuel_prices_array[:, 0] = (
                cap_cost_lc + var_op_cost_lc + fix_op_cost_lc + elec_prices + co2_costs
        )
        electrofuel_prices_array[:, 1] = cap_cost_lc  # CAPEX
        electrofuel_prices_array[:, 2] = fix_op_cost_lc  # FIX_OPEX
        electrofuel_prices_array[:, 3] = var_op_cost_lc  # VAR_OPEX
        electrofuel_prices_array[:, 4] = elec_prices  # ELECTRICITY
        electrofuel_prices_array[:, 5] = co2_costs  # CO2

        return electrofuel_prices_array
