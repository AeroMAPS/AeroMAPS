...
class BiofuelAbatementPotential(AeromapsModel):
    ...
    def compute(
        ...
        energy_avail_atj = aviation_biomass_allocated_share / 100 * available_biomass_atj * 10 ** 12 * biofuel_atj_efficiency

        abatement_potential_atj = energy_avail_atj * (
        ...
        energy_avail_hefa_fog = aviation_biomass_allocated_share / 100 * available_biomass_hefa_fog * 10 ** 12 * biofuel_hefa_fuel_efficiency
        abatement_potential_hefa_fog = energy_avail_hefa_fog * (
                kerosene_emission_factor - biofuel_hefa_fog_emission_factor) / 1000000

        energy_avail_hefa_others = aviation_biomass_allocated_share / 100 * available_biomass_hefa_others * 10 ** 12 * biofuel_hefa_oil_efficiency * biofuel_hefa_fuel_efficiency
        abatement_potential_hefa_others = energy_avail_hefa_others * (
                kerosene_emission_factor - biofuel_hefa_others_emission_factor) / 1000000

        energy_avail_ft_msw = aviation_biomass_allocated_share / 100 * available_biomass_ft_msw * 10 ** 12 * biofuel_ft_efficiency
        abatement_potential_ft_msw = energy_avail_ft_msw * (
                kerosene_emission_factor - biofuel_ft_msw_emission_factor) / 1000000

        energy_avail_ft_others = aviation_biomass_allocated_share / 100 * available_biomass_ft_others * 10 ** 12 * biofuel_ft_efficiency
        abatement_potential_ft_others = energy_avail_ft_others * (
                kerosene_emission_factor - biofuel_ft_others_emission_factor) / 1000000

        h2_avoided_emissions_factor = (
                kerosene_emission_factor / hydrogen_replacement_ratio - hydrogen_electrolysis_emission_factor)
        energy_avail_hydrogen_electrolysis = aviation_available_electricity * electricity_hydrogen_consumption / (
                electricity_electrofuel_consumption + electricity_hydrogen_consumption) * 10 ** 12 * electrolysis_efficiency * \
                                                    liquefaction_efficiency
        abatement_potential_hydrogen_electrolysis = energy_avail_hydrogen_electrolysis * h2_avoided_emissions_factor / 1000000
        
        

        ## The same electricity consumption share (hydrogen/electrofuel) is kept for maxiaml abatement potential computation
        
        energy_avail_electrofuel = aviation_available_electricity * electricity_electrofuel_consumption / (
                electricity_electrofuel_consumption + electricity_hydrogen_consumption) * 10 ** 12 * electrolysis_efficiency \
                                          * electrofuel_hydrogen_efficiency
        abatement_potential_electrofuel = energy_avail_electrofuel * (
        ...
        self.df.loc[:, "abatement_potential_hydrogen_electrolysis"] = abatement_potential_hydrogen_electrolysis
        ...
        abatement_effective_hefa_fog = biomass_hefa_fog_consumption * 10 ** 12 * biofuel_hefa_fuel_efficiency * (
                kerosene_emission_factor - biofuel_hefa_fog_emission_factor) / 1000000

        abatement_effective_hefa_others = biomass_hefa_others_consumption * 10 ** 12 * biofuel_hefa_oil_efficiency * biofuel_hefa_fuel_efficiency * (
                kerosene_emission_factor - biofuel_hefa_others_emission_factor) / 1000000

        abatement_effective_ft_msw = biomass_ft_consumption * (biofuel_ft_msw_share/(biofuel_ft_others_share+biofuel_ft_msw_share)) * 10 ** 12 * biofuel_ft_efficiency * (
                kerosene_emission_factor - biofuel_ft_msw_emission_factor) / 1000000

        abatement_effective_ft_others = biomass_ft_consumption * (biofuel_ft_others_share/(biofuel_ft_others_share+biofuel_ft_msw_share)) * 10 ** 12 * biofuel_ft_efficiency * (
                kerosene_emission_factor - biofuel_ft_others_emission_factor) / 1000000

        h2_avoided_emissions_factor = (
                kerosene_emission_factor / hydrogen_replacement_ratio - hydrogen_electrolysis_emission_factor)

        abatement_effective_hydrogen_electrolysis = electricity_hydrogen_consumption * 10 ** 12 * electrolysis_efficiency * \
                                                    liquefaction_efficiency * h2_avoided_emissions_factor / 1000000

        abatement_effective_electrofuel = electricity_electrofuel_consumption * 10 ** 12 * electrolysis_efficiency * \
                                          electrofuel_hydrogen_efficiency * (
                                                      kerosene_emission_factor - electrofuel_emission_factor) / 1000000
