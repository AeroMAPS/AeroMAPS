...
class RPKWithElasticity(AeroMAPSModel):
    ...
    def compute(
        ...
        airfare_init = 0.09278766071704128  # See total_airline_cost_and_airfare ==> IATA base value calibration

        self.df.loc[covid_end_year_passenger + 1 : self.end_year + 1, "rpk"] = (
            rpk_no_elasticity
            / (airfare_init**price_elasticity)
            * (
                airfare_per_rpk.loc[covid_end_year_passenger + 1 : self.end_year + 1]
                ** price_elasticity
            )
        )
