...
class PassengerAircraftEfficiencyComplex(AeromapsModel):
    ...
    def compute(
        ...
        for k in range(self.prospection_start_year, self.end_year + 1):
            self.df.loc[k, "energy_per_ask_without_operations_short_range_dropin_fuel"] = 0
            self.df.loc[k, "energy_per_ask_without_operations_medium_range_dropin_fuel"] = 0
            self.df.loc[k, "energy_per_ask_without_operations_long_range_dropin_fuel"] = 0
        ...
        for k in range(self.prospection_start_year + 1, self.end_year + 1):
            self.df.loc[k, "energy_per_ask_without_operations_short_range_hydrogen"] = 0
            self.df.loc[k, "energy_per_ask_without_operations_medium_range_hydrogen"] = 0
            self.df.loc[k, "energy_per_ask_without_operations_long_range_hydrogen"] = 0
        ...
        ask_short_range_dropin_fuel_share = 50
        ask_medium_range_dropin_fuel_share = 50
        ask_long_range_dropin_fuel_share = 50
        ask_short_range_hydrogen_share = 50
        ask_medium_range_hydrogen_share = 50
        ask_long_range_hydrogen_share = 50

        self.df.loc[:, "ask_short_range_dropin_fuel_share"] = ask_short_range_dropin_fuel_share
        self.df.loc[:, "ask_medium_range_dropin_fuel_share"] = ask_medium_range_dropin_fuel_share
        self.df.loc[:, "ask_long_range_dropin_fuel_share"] = ask_long_range_dropin_fuel_share
        self.df.loc[:, "ask_short_range_hydrogen_share"] = ask_short_range_hydrogen_share
        self.df.loc[:, "ask_medium_range_hydrogen_share"] = ask_medium_range_hydrogen_share
        self.df.loc[:, "ask_long_range_hydrogen_share"] = ask_long_range_hydrogen_share

        # ASK

        ask_short_range_dropin_fuel = ask_short_range * ask_short_range_dropin_fuel_share / 100
        ask_medium_range_dropin_fuel = ask_medium_range * ask_medium_range_dropin_fuel_share / 100
        ask_long_range_dropin_fuel = ask_long_range * ask_long_range_dropin_fuel_share / 100
        ask_short_range_hydrogen = ask_short_range * ask_short_range_hydrogen_share / 100
        ask_medium_range_hydrogen = ask_medium_range * ask_medium_range_hydrogen_share / 100
        ask_long_range_hydrogen = ask_long_range * ask_long_range_hydrogen_share / 100
