...
class AircraftGrowthModel(LogisticFunctionYearSeries):
    def __init__(self, name="aircraft_growth", *args, **kwargs):
    ...
    def compute(
        ...
        share: float = 0.0,
        life: float = 0.0,
        entry_into_service_year: int = 0,
    ):
        self._update_parameters(life, share, entry_into_service_year)
        self._compute()

    def _compute(self, maximum_share=100):
        self._update_parameters()
        y_share = super()._compute()
        y_share_max = super()._compute(maximum_value=maximum_share)
        y = np.where(y_share < self.limit, 0.0, y_share_max)
        self.df[self.name] = y
        return self.df[self.name]

    def _update_parameters(self, life, share, entry_into_service_year):
        self.limit = 2
        self.growth_rate = np.log(100 / self.limit - 1) / (life / 2)
        self.maximum_value = share
        self.midpoint_year = entry_into_service_year + life / 2


class FleetGrowthModel(AeromapsModel):
    def __init__(self, name="fleet_growth", fleet=None, *args, **kwargs):
        super().__init__(name, *args, **kwargs)
        self.fleet = fleet
    ...
    def compute(
        ...
        # Single aircraft share computation
        for category in self.fleet.categories.values():
            for subcategory in category.subcategories.values():
                ...
                ref_recent_aircraft_share = self._compute(
        ...
        for category in self.fleet.categories.values():
            ...
            for subcategory in category.subcategories.values():
                ...
                # Share of dropin aircraft in subcategory
                self.df[
