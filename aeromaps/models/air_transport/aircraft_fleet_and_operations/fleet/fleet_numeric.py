    ...
    def compute(
        ...
        for category, sets in self.fleet_model.all_aircraft_elements.items():
            ...
            for year in range(covid_start_year, covid_end_year + 1):
                category_ask_covid_levelling.loc[year] = ((category_ask_pre_covid - category_ask_post_covid) / (
                        covid_start_year - 1) - (covid_end_year)) * (
            ...
            for aircraft_var in sets:
                ...
                share_var_name = (
                        aircraft_var_name + ":aircraft_share"
                )
                ask_aircraft_var_name = (
                        aircraft_var_name + ":aircraft_ask"
                )

                aircraft_in_fleet_var_name = (
                        aircraft_var_name + ":aircraft_in_fleet"
                )
                ...
                ask_aircraft_value_covid_levelling = self.fleet_model.df.loc[2019:  2050,
                                                     share_var_name] / 100 * category_ask_covid_levelling
                ...
                aircraft_in_fleet_value_covid_levelling_dict[aircraft_var_name] = aircraft_in_fleet_value_covid_levelling
