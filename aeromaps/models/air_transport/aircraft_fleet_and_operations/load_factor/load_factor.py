...
``LoadFactor``           — legacy global model.
``LoadFactorMarket``     — per-market load factor for the market refactor path.
...
.. note::
    PHASE-5-CLEANUP: ``LoadFactor`` is superseded by ``LoadFactorMarket`` +
    ``LoadFactorAggregator``. Delete it (and its registration in ``models.py``)
...
class LoadFactor(AeroMAPSModel):
    ...
    def compute(
        ...
        a, b = _parameters_load_factor_model(self.end_year, load_factor_2019, load_factor_end_year)
    ...
    def parameters_load_factor_model(end_year, load_factor_2019, load_factor_end_year):
        # PHASE-5-CLEANUP: thin wrapper kept for backwards compatibility while
        # ``LoadFactor`` is alive. Delete with the class.
        return _parameters_load_factor_model(end_year, load_factor_2019, load_factor_end_year)
