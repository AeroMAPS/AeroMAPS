...
def test_doc_breakdown_groups_pathways(process):
    """Pathways mapped to one group label are drawn, and named, as one band."""
    from matplotlib.legend import Legend

    from aeromaps.plots.single_scenario.costs_generic import NetEnergyDOCPerRPKBreakdown

    ungrouped = NetEnergyDOCPerRPKBreakdown(process)
    active = [p.name for p in ungrouped._get_active_pathways()]
    groups = {name: "All drop-in fuels" for name in active}
    grouped = NetEnergyDOCPerRPKBreakdown(
        process, groups=groups, group_colors={"All drop-in fuels": "grey"}
    )

    pathways, data, colors = grouped._prepare()
    assert [p.name for p in pathways] == ["All drop-in fuels"]
    assert colors["All drop-in fuels"] == "grey"

    # Grouping moves no cost: each component sums to the ungrouped total.
    _, raw, _ = ungrouped._prepare()
    for component, values in data["All drop-in fuels"].items():
        total = sum(raw[name][component] for name in active)
        assert abs(values - total).max() < 1e-12

    carrier_legends = [
        child
        for child in grouped.ax.get_children()
