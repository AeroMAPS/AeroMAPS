...
def test_auto_model_wrapper_inputs_outputs():
    ...
    assert all(wrapper.input_grammar[key] is float for key in ["x", "y"])
    assert all(wrapper.output_grammar[key] is float for key in ["z", "product"])
...
def test_auto_model_wrapper_execution():
    ...
    product1_val = (
        float(product1)
        if hasattr(product1, "__iter__") and not isinstance(product1, str)
        else float(product1)
    )
    ...
    product2_val = (
        float(product2)
        if hasattr(product2, "__iter__") and not isinstance(product2, str)
        else float(product2)
    )
