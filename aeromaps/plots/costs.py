...
class ScenarioEnergyExpensesPlot:
    ...
    def plot_interact(self):
        pathway_widget = widgets.SelectMultiple(
            options=["Overview"] + [pathway.name for pathway in self.pathways_manager.get_all()],
            description="Energy carrier:",
            value=["Overview"],
        )

        interact(self.update, pathway=pathway_widget)
    ...
    def update(self, pathway):
        ...
        if "Overview" in pathway:
            ...
            self.ax.legend(
                np.array([p.name for p in self.pathways_manager.get_all()]),
                loc="upper left",
                prop={"size": 7},
            )
        else:
            self.ax.stackplot(
                ...
                np.array(
                    [
                        self.df.loc[self.prospective_years, f"{p}_mfsp"].fillna(0).values
                        * self.df.loc[self.prospective_years, f"{p}_energy_consumption"]
                        .fillna(0)
                        .values
                        / 1e6
                        for p in pathway
            ...
            self.ax.legend(pathway, loc="upper left", prop={"size": 7})
