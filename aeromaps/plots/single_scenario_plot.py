...
class SingleScenarioPlot(ABC):
    ...
    def _apply_legend_setting(self):
        ...
        if legend is False:
            # A plot may place more than one legend on its axes, keeping the
            # earlier ones alive through add_artist; ``get_legend`` returns only
            # the last, so hiding just that leaves the others drawn and
            # ``legend=False`` silently half-works.
            from matplotlib.legend import Legend

            for artist in [self.ax.get_legend(), *self.ax.artists]:
