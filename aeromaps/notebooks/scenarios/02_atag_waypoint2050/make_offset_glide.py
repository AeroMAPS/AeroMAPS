...
# scenario -> (inputs file, outputs file it takes its gross trajectory from)
SCENARIOS = {
    "3rd edition full S1": ("3rd_edition_full", "s1_inputs.json", "s1.json"),
    "3rd edition full S2": ("3rd_edition_full", "s2_inputs.json", "s2.json"),
    "3rd edition light S0": ("3rd_edition_light", "s0_inputs.json", "s0.json"),
    "3rd edition light S1": ("3rd_edition_light", "s1_inputs.json", "s1.json"),
    "3rd edition light S2": ("3rd_edition_light", "s2_inputs.json", "s2.json"),
}
...
def main():
    ...
    for label, (edition, inputs_name, outputs_name) in SCENARIOS.items():
        inputs_path = here / edition / "data_inputs" / inputs_name
