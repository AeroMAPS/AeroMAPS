...
This decomposition requires the generic offsets module, activated by the `models.offsets` block of the configuration 
file, which then replaces the simple offset models and their single price. The emissions left after the levers of 
action are offset by schemes (an emissions trading system, CORSIA, a removals obligation...), each with its own 
quantity rule and its own price $p_s(t)$. Three rules are available:

- **level**: the emissions above a baseline set at a fraction $b_s(t)$ of the emissions of a reference year 
$t_{ref}$, times the fraction $k_s(t)$ of emissions in the scope of the scheme, as in CORSIA:

$$
Q_s(t) = \max \big( CO_2(t) - b_s(t) \, CO_2(t_{ref}), \, 0 \big) \, k_s(t)
$$

- **share of residual**: a fraction $\sigma_s(t)$ of the emissions left once the level-based schemes are applied:

$$
Q_s(t) = \sigma_s(t) \Big[ CO_2(t) - \sum_{s' \in \text{level}} Q_{s'}(t) \Big]
$$

- **quantity**: a prescribed annual quantity $Q_s(t)$.

The carbon offset is the sum of the scheme quantities, so the sub-levers carry no residual, and its price is the 
quantity-weighted mean of the scheme prices:

$$
Q(t) = \sum_s Q_s(t), \qquad p(t) = \frac{\sum_s Q_s(t) \, p_s(t)}{Q(t)}
$$

The single-price cost chain of offsetting is therefore reproduced exactly, while the expense is also available per 
scheme and per category of schemes. The rules reproduce those of the simple offset models, including one of their 
limits: prescribed quantities are not deducted from the residual that the share-based schemes apply to. A warning is 
issued when the schemes offset more than the emissions, or when the shares of residual exceed 100 %.
...
| Carbon offset per scheme | `OffsetsUseChoice` | the generic offsets module, when `models.offsets` is declared |
...
`co2_emissions_lever_operations_concept_<concept>`, `co2_emissions_lever_energy_pathway_<pathway>`, 
`co2_emissions_lever_offset_scheme_<scheme>` or `co2_emissions_lever_energy_market_<market>`; residuals end with `_other` or `_market_cross_mix`. Values below 
...
- `carbon_offset_by_scheme`, `carbon_offset_by_category` and `carbon_offset_expense_by_scheme` show the offset 
...
emissions). Values for fossil kerosene from [@lee2021contribution] have been used and are provided in Tab.1. The
...
through an analysis of the data from [@lee2021contribution] and are summarized in Tab.2. CO<sub>2</sub> emissions
...
literature to estimate them [@aamaas2013simple; @shine2005alternatives]: GWP, AGWP, GTP, AGTP, etc.
...
[@lynch2020demonstrating], which is the case for aviation non-CO<sub>2</sub> effects. As a consequence, an
alternative metric is used here. GWP</sup>*</sup> is an improved climate metric developed recently 
[@allen2018solution; @cain2019improved; @collins2020stable], one of the major interests of which is to better
...
[@meinshausen2022gwp]. Compared to GWP, it does not only take into account the absolute value of the emissions but
...
This equation can be modified to be used for aviation non-CO<sub>2</sub> effects, based on assumptions from [@allen2018solution; @lee2021contribution],
with in particular $r=1$ and $s=0$. As a consequence, the following
...
following equation is used with $T_{2019}$ the temperature increase from air transport in 2019 (from [@grewe2021evaluating]),
$E_{CO_2, k}$ the annual CO<sub>2</sub> emissions and $E_{CO_2\text{-}we, k}$ the annual
...
models is first provided. Alternative energy cost models were presented in detail in [@salgas2023cost], with a model taking as an
input each fuel pathway annual energy consumption to derive annual energy expenses and the required investments 
chronology for the energy sector. A direct operating cost model adapted to AeroMAPS was recalibrated in [@salgas2023tlars]
and adapted to AeroMAPS in [@salgas2023regulations].
...
identified by the literature [@pavlenko_cost_2019; @de_jong_green_2018]: the capital required to build a conversion plant (Capital Expenditures  CapEx),
...
In AeroMAPS, the biofuel MFSP were directly taken from a literature review [@irena_reaching_2021; @pavlenko_cost_2019; @de_jong_green_2018], and the CapEx
...
Reference technological values on CapEx, OpEx and efficiencies are taken from [@uk_department_for_business_energy_and_industrial_strategy_hydrogen_2021; @pik_price_2022; @international_energy_agency_global_2021]. When hydrogen is directly
...
By default, a fictional carbon tax implementing in full the French Value for Climate Action [@quinet_what_2020] is used.
...
Explicit direct operating cost models are presented in [@risse_central_2016], on a per flight hour basis. These models depend
directly on the aircraft characteristics. They were recalibrated using United States Bureau of Transportation Statistics
data and adapted to a per flight basis in [@salgas2023tlars]. Since AeroMAPS uses a global
...
average values for selected aircraft and distance categories were extracted from the results of [@salgas2023tlars] based on the full
...
This model is under implementation, but a beta version is described in [@salgas2023regulations].
...
the estimation of carbon budgets [@matthews2009proportionality]. Therefore, the use of carbon budgets allows
for example to simply evaluate the relevance of transition scenarios to reach climate objectives based on their 
CO<sub>2</sub> emissions alone [@friedlingstein2014persistent].
...
(technical, economic, societal, etc.). Discussions on this allocation are proposed in [@delbecq2022isae].
...
CO<sub>2</sub> emissions [@delbecq2022isae]. This value can be interpreted as the share that would be allocated
...
trade-off may require multi-sectoral approaches, as for example proposed in [@sbti2015sectoral]. In this scenario,
...
for instance performed in [@grewe2021evaluating; @klower2021quantifying].
...
been adapted from simplified models for estimating carbon budgets described in [@rogelj2019estimating]. The
...
aviation, over a recent period and including global CO<sub>2</sub> emissions and non-CO<sub>2</sub> effects [@delbecq2022isae].
...
The estimates in Tab.4 are refined using an analysis of the references [@staples2017limits; @staples2018aviation]
...
The results obtained are consistent with [@slade2014global], which concludes that likely estimates are less than
300 EJ. Similarly, the results by resource type are of the same order of magnitude as those given in [@o2021estimating].
...
50 gCO<sub>2</sub>-eq/kWh, with values of the order of 10 gCO<sub>2</sub>-eq/kWh for wind or nuclear power [@IPCC-AR5].
