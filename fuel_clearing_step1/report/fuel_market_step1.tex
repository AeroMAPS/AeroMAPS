...
\begin{keybox}
\textbf{So: fuel cost, or compliance, or both?}

\begin{itemize}[leftmargin=*]
  \item At $\boldsymbol{w=0}$ (the default) the airline pays \textbf{the production cost
        of the fuel it buys, and nothing else}. $\lambda^{M}$ is computed and reported
        but \emph{never reaches the airline}. It is a shadow price --- an economic
        diagnostic saying what the obligation is worth --- not a charge.
  \item At $\boldsymbol{w=1}$ the airline pays the marginal price, which by
        \eqref{eq:marginal} \textbf{is} energy price \emph{plus} compliance price on
        eligible pathways. Here compliance is passed through in full.
  \item Between, it pays a $w$-weighted blend.
  \item \textbf{It is never ``compliance only''.} Compliance is a premium \emph{on top
        of} the energy price, not a substitute for it.
  \item The \textbf{carbon tax} is added by \var{DirectOperatingCosts} as its own,
        separate line --- which is precisely why \eqref{eq:marketmfsp} subtracts the tax
        wedge back out (\S\ref{sec:netgross}).
\end{itemize}

\textbf{One thing the airline does \emph{not} pay, and should:} the buy-out payment
$B\,x$ appears in objective~\eqref{eq:objective} but in no price output. If the market
releases part of the obligation, that penalty is currently borne by nobody in the cost
chain. At $w=1$ the \emph{rate} is signalled correctly ($\lambda^{M}$ is capped at $B$),
but the \emph{payment} is not accounted. This is a known gap, not a modelling choice.
\end{keybox}
